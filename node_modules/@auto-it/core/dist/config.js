...
function normalizeLabel(label) {
    const baseLabel = release_1.defaultLabels.find((l) => {
        let isBase = false;
...
function normalizeLabels(config) {
    if (config.labels) {
        ...
        const baseLabels = release_1.defaultLabels.filter((d) => !userLabels.some((u) => u.releaseType && u.releaseType === d.releaseType && u.overwrite));
        return [...userLabels, ...baseLabels];
    }
    return release_1.defaultLabels;
}
...
class Config {
    ...
    async loadConfig() {
        ...
        return Object.assign(Object.assign({}, rawConfig), { labels, prereleaseBranches: rawConfig.prereleaseBranches || ["next"], versionBranches: typeof rawConfig.versionBranches === "boolean"
                ? "version-"
